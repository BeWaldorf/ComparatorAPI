...
req_URL: dict[str, str] = { "BE":   "https://www.amazon.com.be/s?k=",
...
headers: dict = {'authority': 'www.amazon.com',
                    'pragma': 'no-cache',
...
def api_search(country_code, search_term):
    ...
    soup = BeautifulSoup(page.content, "html.parser")
    search_results = soup.find_all(attrs={"data-component-type": "s-search-result"})
    result_items_str:str = scrape_processor(search_results)
    return result_items_str

def scrape_processor(results:list[Tag]) -> list[dict]:
    itemList: list[dict] = []
...
def pretty_results(results: list) -> list[SearchItem]:
